...
class MeasurementProvider(DataProvider):
    """
    This class provides performance measurement for

    a) arbitrary commits (by extrapolating existing measurements)
    b) closest configurations (those most similar to the ones queried)
...
def get_sub_fm(path: str):
    data = pd.read_csv(path, delimiter=";")
    data = data[data.columns[:-4]]
    configs = np.unique(data.values, axis=0)
    config_expressions = []
    for row in configs:
        ...
        for i, fname in enumerate(data.columns):
            ...
            if row[i] == 0:
                f = z3.Not(f)
            clause.append(f)
        clause = z3.And(clause)
        config_expressions.append(clause)
    config_expressions = z3.Or(config_expressions)
    fm = z3.simplify(config_expressions, elim_or=True)
...
print(get_sub_fm("measurements/vp9.csv"))
"""
# Example run
